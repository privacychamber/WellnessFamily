...
interface ContactData {
  ...
  sanctuaryDescription: string;
  locationTitle: string;
  locationDescription: string;
  googleMapsUrl: string;
}

export function ContactUI({ data }: { data: ContactData }) {
  const title = data?.title || "Get in Touch.";
  const description = data?.description || "";
  const formTitle = data?.formTitle || "Send a Message";
  const sanctuaryTitle = data?.sanctuaryTitle || "Our Sanctuary";
  const sanctuaryDescription = data?.sanctuaryDescription || "";
  const locationTitle = data?.locationTitle || "Location";
    ...
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <section className="py-20 px-6 md:px-12 flex flex-col md:flex-row gap-16 max-w-6xl mx-auto">
        ...
        <div className="w-full md:w-1/2 flex flex-col">
          <h2 className="font-serif text-3xl text-forest mb-8">{formTitle}</h2>
        ...
        <div className="w-full md:w-1/2 flex flex-col gap-12">
          <div>
            <h2 className="font-serif text-3xl text-forest mb-6">{sanctuaryTitle}</h2>
            <p className="font-sans text-lg text-forest/80 mb-4 leading-relaxed whitespace-pre-line">
          ...
          <div className="bg-[#f5f1e7] p-8 flex flex-col items-center justify-center text-center rounded-sm">
            ...
            <h3 className="font-serif text-2xl text-forest mb-4">{locationTitle}</h3>
            <p className="font-sans text-forest/70 mb-8 max-w-sm whitespace-pre-line">
              {locationDescription}
            </p>
